...
float circle( vec2 uv, float thick, float delay ) {
	...
	float t = sin(iGlobalTime*2.0 + delay)/2.+0.5;
	float ease = elasticInOut(t);
	float anim = 30.0 + ease * 40.0;
	
	
	float size = 0.0 + anim * 0.5;
	...
	float s2 = size-thick * 0.5;
	float c2 = smoothstep(s2/res, s2/res + 2.0/res, dist);
	
	vec2 norm = 2.0 * uv - 1.0;
	float phi = atan(norm.y, norm.x)/PI + iGlobalTime*0.75;
	float a = fract(phi);
...
void main( void ) {
    ...
    //vec2 aspect = vec2(iResolution.x/iResolution.y, 1.0);
  	//vec2 uv = aspect*(gl_FragCoord.xy / iResolution);
	...
	pos /= (max(iResolution.x, iResolution.y)*0.76)/iResolution; //scale it
	pos -= vec2(-0. + 100.0/iResolution.x, 1.0); //offset
	...
	color = mix(color, second, sin(iGlobalTime*0.75)/2.+.5);
